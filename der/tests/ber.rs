...
#[test]
fn test_parse_ber() {
    let bytes_ber = &[0x30, 0x80, 0x02, 0x01, 0x42, 0x02, 0x01, 0x43, 0x0c, 0x02, 0x48, 0x69, 0x00, 0x00];
...
fn test_parse_ber_string_indefinite() {
    let bytes_ber = &[0x30, 0x80, 0x02, 0x01, 0x42, 0x02, 0x01, 0x43, 0x0c, 0x80, 0x0c, 0x02, 0x48, 0x69, 0x00, 0x00, 0x00, 0x00];
    let point: Point = der::Decode::from_ber(bytes_ber.as_slice()).unwrap();
    println!("x: {}, y: {}, name: {:?}", point.x, point.y, point.name);
}
...
fn test_parse_ber_string_indefinite_and_constructed() {
    let bytes_ber = &[0x30, 0x80, 0x02, 0x01, 0x42, 0x02, 0x01, 0x43, 0x0c, 0x80, 0x0c, 0x02, 0x48, 0x69, 0x0c, 0x02, 0x48, 0x69, 0x00, 0x00, 0x00, 0x00];
    let point: Point = der::Decode::from_ber(bytes_ber.as_slice()).unwrap();
    println!("x: {}, y: {}, name: {:?}", point.x, point.y, point.name);
}
